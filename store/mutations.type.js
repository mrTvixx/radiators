export const START_FETCH = "START_FETCH";
export const END_FETCH = "END_FETCH";
export const SAVE_SEARCH_PRODUCTS = "SAVE_SEARCH_PRODUCTS";
export const CLEAR_SEARCH_PRODUCTS = "CLEAR_SEARCH_PRODUCTS";
export const SAVE_PRODUCTS_LIST = "SAVE_PRODUCTS_LIST";
export const CLEAR_PRODUCTS_LIST = "CLEAR_PRODUCTS_LIST";
export const SAVE_SEARCH_VALUE = "SAVE_SEARCH_VALUE";
export const SAVE_PRODUCT_DATA = "SAVE_PRODUCT_DATA";
export const CLEAR_PRODUCT_DATA = "CLEAR_PRODUCT_DATA";
export const ADD_TO_CART = "ADD_TO_CART";
export const REMOVE_FROM_CART = "REMOVE_FROM_CART";
export const CHECK_CART_DATA = "CHECK_CART_DATA";
export const CHANGE_PRODUCT_COUNT = "CHANGE_PRODUCT_COUNT";
export const SAVE_ORDER_ID = "SAVE_ORDER_ID";
export const SAVE_ORDER_DATA = "SAVE_ORDER_DATA";
export const CLEAR_ORDER_DATA = "CLEAR_ORDER_DATA";
export const SAVE_FILTERS_LIST = "SAVE_FILTERS_LIST";
export const CLEAR_FILTERS_LIST = "CLEAR_FILTERS_LIST";
export const SET_PAGINATION_PAGE = "SET_PAGINATION_PAGE";
export const SAVE_MANUFACTURERS_LIST = 'SAVE_MANUFACTURERS_LIST';
export const SAVE_CATALOG_LIST = 'SAVE_CATALOG_LIST';
